...
public class Ajida {
	...
	public static void htmlProjectUpdate(boolean needGitPull,String even,SSHConfig sshConfig,String projectLocalDir,String remoteProjectDir) throws Exception{
		...
		try {
			...
			cmds = new String[]{
					"cd "+projectLocalDir,
					projectLocalDir.substring(0,2),
					"copy "+projectLocalDir+"\\config\\"+even+"\\*.*"+" "+projectLocalDir+"\\"+projectName+"\\"
			};
			CmdUtil.exec(cmds);
	 ...
	 * @throws Exception 
	 */
	public static void mvnPackageJarApplication(String projectPath,String configPath,AxeAppConfig appConfig,String zipName) throws Exception{
		try {
